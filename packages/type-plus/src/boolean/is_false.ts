...
export namespace IsFalse {
	 ...
	 * 🧰 *type function*
	 *
	 * `IsFalse` as a type function, with its options `$O` applied.
	 *
	 * @example
	 * ```ts
	 * type R = $Fn.Apply<IsFalse.$Fn, false> // true
	 * type R = $Fn.Apply<IsFalse.$Fn, true> // false
	 * ```
	 */
	export interface $Fn<$O extends $StrictOptions<$O, $Options> = {}> extends $FnBase {
		readonly out: IsFalse<this['in'], $O>
	}
