...
import type { IsStrictFunction } from './is_strict_function.js'
 ...
 * 🔢 *customize*
 *
 * Check if `T` is exactly `Function`, the same as `IsStrictFunction`.
 *
 * @example
 * ```ts
...
export namespace IsFunction {
	...
	export type $<T, $O extends $UtilOptions> = $ResolveOptions<[$O['exact'], $Exact.Default['exact']]> extends true
		? $Distributive.Parse<
				$O,
				{ $then: IsStrictFunction._D<T, $O>; $else: IsEqual._ExactEqualNonDistributive<T, Function, $O> }
			>
