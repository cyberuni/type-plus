...
import { type $Else, type $Fn, type $Then, type IsNumber, testType } from '../index.js'
...
describe('IsNumber.$Fn', () => {
	it('is IsNumber as a type function', () => {
		testType.equal<$Fn.Apply<IsNumber.$Fn, 1>, true>(true)
		testType.equal<$Fn.Apply<IsNumber.$Fn, 'a'>, false>(true)
	})
})

it('resolves `IsNumber.$Default` the same as no options', () => {
	// `IsNumber.$Default` documents the default; the type never reads it, so pin the two together.
	testType.equal<IsNumber<any, IsNumber.$Default>, IsNumber<any>>(true)
	testType.equal<IsNumber<unknown, IsNumber.$Default>, IsNumber<unknown>>(true)
	testType.equal<IsNumber<never, IsNumber.$Default>, IsNumber<never>>(true)
	testType.equal<IsNumber<void, IsNumber.$Default>, IsNumber<void>>(true)
	testType.equal<IsNumber<undefined, IsNumber.$Default>, IsNumber<undefined>>(true)
	testType.equal<IsNumber<null, IsNumber.$Default>, IsNumber<null>>(true)
	testType.equal<IsNumber<boolean, IsNumber.$Default>, IsNumber<boolean>>(true)
	testType.equal<IsNumber<true, IsNumber.$Default>, IsNumber<true>>(true)
	testType.equal<IsNumber<1, IsNumber.$Default>, IsNumber<1>>(true)
	testType.equal<IsNumber<number, IsNumber.$Default>, IsNumber<number>>(true)
	testType.equal<IsNumber<'a', IsNumber.$Default>, IsNumber<'a'>>(true)
	testType.equal<IsNumber<string, IsNumber.$Default>, IsNumber<string>>(true)
	testType.equal<IsNumber<symbol, IsNumber.$Default>, IsNumber<symbol>>(true)
	testType.equal<IsNumber<1n, IsNumber.$Default>, IsNumber<1n>>(true)
	testType.equal<IsNumber<{}, IsNumber.$Default>, IsNumber<{}>>(true)
	testType.equal<IsNumber<[], IsNumber.$Default>, IsNumber<[]>>(true)
	testType.equal<IsNumber<() => void, IsNumber.$Default>, IsNumber<() => void>>(true)
	testType.equal<IsNumber<1 | string, IsNumber.$Default>, IsNumber<1 | string>>(true)
})
