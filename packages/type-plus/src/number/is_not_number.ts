...
export namespace IsNotNumber {
	 ...
	 * 🧰 *type function*
	 *
	 * `IsNotNumber` as a type function, with its options `$O` applied.
	 *
	 * Prefer it over `$Fn.Not<IsNumber.$Fn>`: it costs less.
	 *
	 * @example
	 * ```ts
	 * type R = $Fn.Apply<IsNotNumber.$Fn, 'a'> // true
	 * type R = $Fn.Apply<IsNotNumber.$Fn, 1> // false
	 * ```
	 */
	export interface $Fn<$O extends $StrictOptions<$O, $Options> = {}> extends $FnBase {
		readonly out: IsNotNumber<this['in'], $O>
	}
