...
import type { _IsFraction } from './_numeric-fraction.js'
...
export namespace IsNotIntegerLiteral {
	 ...
	 * 🧰 *type function*
	 *
	 * `IsNotIntegerLiteral` as a type function, with its options `$O` applied.
	 *
	 * Prefer it over `$Fn.Not<IsIntegerLiteral.$Fn>`: it costs less.
	 *
	 * @example
	 * ```ts
	 * type R = $Fn.Apply<IsNotIntegerLiteral.$Fn, number> // true
	 * type R = $Fn.Apply<IsNotIntegerLiteral.$Fn, 1> // false
	 * ```
	 */
	export interface $Fn<$O extends $StrictOptions<$O, $Options> = {}> extends $FnBase {
		readonly out: IsNotIntegerLiteral<this['in'], $O>
	}
