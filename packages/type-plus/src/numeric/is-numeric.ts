...
export namespace IsNumeric {
	 ...
	 * 🧰 *type function*
	 *
	 * `IsNumeric` as a type function, with its options `$O` applied.
	 *
	 * @example
	 * ```ts
	 * type R = $Fn.Apply<IsNumeric.$Fn, 1n> // true
	 * type R = $Fn.Apply<IsNumeric.$Fn, 'a'> // false
	 * ```
	 */
	export interface $Fn<$O extends $StrictOptions<$O, $Options> = {}> extends $FnBase {
		readonly out: IsNumeric<this['in'], $O>
	}
