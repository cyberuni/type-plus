...
export namespace IsPositiveLiteral {
	 ...
	 * 🧰 *type function*
	 *
	 * `IsPositiveLiteral` as a type function, with its options `$O` applied.
	 *
	 * @example
	 * ```ts
	 * type R = $Fn.Apply<IsPositiveLiteral.$Fn, 1> // true
	 * type R = $Fn.Apply<IsPositiveLiteral.$Fn, number> // false
	 * ```
	 */
	export interface $Fn<$O extends $StrictOptions<$O, $Options> = {}> extends $FnBase {
		readonly out: IsPositiveLiteral<this['in'], $O>
	}
