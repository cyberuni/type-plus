/**
 * Pins the `@example` blocks in the `src/numeric/*.ts` TSDoc comments to the
 * actual behavior.
 *
 * Every assertion here mirrors a line documented in `src/numeric/*.ts`, so a
...
it('IsNumeric examples in TSDoc are accurate', () => {
	...
	testType.equal<IsNumeric<'1'>, false>(true)
...
it('IsNotNumeric examples in TSDoc are accurate', () => {
	testType.equal<IsNotNumeric<'1'>, true>(true)
...
it('NumericPlus examples in TSDoc are accurate', () => {
