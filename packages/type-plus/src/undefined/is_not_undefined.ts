...
export namespace IsNotUndefined {
	 ...
	 * 🧰 *type function*
	 *
	 * `IsNotUndefined` as a type function, with its options `$O` applied.
	 *
	 * Prefer it over `$Fn.Not<IsUndefined.$Fn>`: it costs less.
	 *
	 * @example
	 * ```ts
	 * type R = $Fn.Apply<IsNotUndefined.$Fn, null> // true
	 * type R = $Fn.Apply<IsNotUndefined.$Fn, undefined> // false
	 * ```
	 */
	export interface $Fn<$O extends $StrictOptions<$O, $Options> = {}> extends $FnBase {
		readonly out: IsNotUndefined<this['in'], $O>
	}
