...
export namespace IsNotNull {
	 ...
	 * 🧰 *type function*
	 *
	 * `IsNotNull` as a type function, with its options `$O` applied.
	 *
	 * Prefer it over `$Fn.Not<IsNull.$Fn>`: it costs less.
	 *
	 * @example
	 * ```ts
	 * type R = $Fn.Apply<IsNotNull.$Fn, undefined> // true
	 * type R = $Fn.Apply<IsNotNull.$Fn, null> // false
	 * ```
	 */
	export interface $Fn<$O extends $StrictOptions<$O, $Options> = {}> extends $FnBase {
		readonly out: IsNotNull<this['in'], $O>
	}
