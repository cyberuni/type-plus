...
import { type $Else, type $Fn, type $Never, type $Then, type $Unknown, type IsNotAny, testType } from '../index.js'
...
describe('IsNotAny.$Fn', () => {
	it('is IsNotAny as a type function', () => {
		testType.equal<$Fn.Apply<IsNotAny.$Fn, 1>, true>(true)
		testType.equal<$Fn.Apply<IsNotAny.$Fn, any>, false>(true)
	})
})

it('resolves `IsNotAny.$Default` the same as no options', () => {
	// `IsNotAny.$Default` documents the default; the type never reads it, so pin the two together.
	testType.equal<IsNotAny<any, IsNotAny.$Default>, IsNotAny<any>>(true)
	testType.equal<IsNotAny<unknown, IsNotAny.$Default>, IsNotAny<unknown>>(true)
	testType.equal<IsNotAny<never, IsNotAny.$Default>, IsNotAny<never>>(true)
	testType.equal<IsNotAny<void, IsNotAny.$Default>, IsNotAny<void>>(true)
	testType.equal<IsNotAny<undefined, IsNotAny.$Default>, IsNotAny<undefined>>(true)
	testType.equal<IsNotAny<null, IsNotAny.$Default>, IsNotAny<null>>(true)
	testType.equal<IsNotAny<boolean, IsNotAny.$Default>, IsNotAny<boolean>>(true)
	testType.equal<IsNotAny<true, IsNotAny.$Default>, IsNotAny<true>>(true)
	testType.equal<IsNotAny<1, IsNotAny.$Default>, IsNotAny<1>>(true)
	testType.equal<IsNotAny<number, IsNotAny.$Default>, IsNotAny<number>>(true)
	testType.equal<IsNotAny<'a', IsNotAny.$Default>, IsNotAny<'a'>>(true)
	testType.equal<IsNotAny<string, IsNotAny.$Default>, IsNotAny<string>>(true)
	testType.equal<IsNotAny<symbol, IsNotAny.$Default>, IsNotAny<symbol>>(true)
	testType.equal<IsNotAny<1n, IsNotAny.$Default>, IsNotAny<1n>>(true)
	testType.equal<IsNotAny<{}, IsNotAny.$Default>, IsNotAny<{}>>(true)
	testType.equal<IsNotAny<[], IsNotAny.$Default>, IsNotAny<[]>>(true)
	testType.equal<IsNotAny<() => void, IsNotAny.$Default>, IsNotAny<() => void>>(true)
	testType.equal<IsNotAny<1 | string, IsNotAny.$Default>, IsNotAny<1 | string>>(true)
})
