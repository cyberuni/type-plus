 ...
 */
export type IsString<T, $O extends $StrictOptions<$O, IsString.$Options> = {}> = [keyof $O] extends [never]
	? $Special.Values<
			T,
			{ $any: false; $unknown: false; $never: false; $void: false; $else: T extends string ? true : false }
		>
	: $Special<
			T,
			$MergeOptions<
...
export namespace IsString {
	 ...
	 * 🧰 *type function*
	 *
	 * `IsString` as a type function, with its options `$O` applied.
	 *
	 * @example
	 * ```ts
	 * type R = $Fn.Apply<IsString.$Fn, 'a'> // true
	 * type R = $Fn.Apply<IsString.$Fn, 1> // false
	 * ```
	 */
