...
export namespace IsBigintLiteral {
	 ...
	 * 🧰 *type function*
	 *
	 * `IsBigintLiteral` as a type function, with its options `$O` applied.
	 *
	 * @example
	 * ```ts
	 * type R = $Fn.Apply<IsBigintLiteral.$Fn, 1n> // true
	 * type R = $Fn.Apply<IsBigintLiteral.$Fn, bigint> // false
	 * ```
	 */
	export interface $Fn<$O extends $StrictOptions<$O, $Options> = {}> extends $FnBase {
		readonly out: IsBigintLiteral<this['in'], $O>
	}
